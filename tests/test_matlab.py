...
def test_octave_build(build_sys, tmp_path):
    subprocess.check_call([OCTAVE, "--eval", f"build('{build_sys}', '{R.parent}', '{tmp_path}')"], cwd=Rcode, timeout=120)


@pytest.mark.skipif(not MATLAB, reason="Matlab not available")
@pytest.mark.parametrize("build_sys", ["cmake", "meson"])
def test_matlab_build(build_sys, tmp_path):
    subprocess.check_call([MATLAB, "-batch", f"build('{build_sys}', '{R.parent}', '{tmp_path}')"], cwd=Rcode, timeout=120)
